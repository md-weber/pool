...
func serializeOrderTlvData(w io.Writer, o order.Order) error {
	...
	if len(o.Details().AllowedNodeIDs) > 0 {
		bytes := FlattenPubKeySlice(o.Details().AllowedNodeIDs)
		tlvRecords = append(
			tlvRecords, tlv.MakePrimitiveRecord(
				allowedNodeIDsType, &bytes,
			),
		)
	}

	if len(o.Details().NotAllowedNodeIDs) > 0 {
		bytes := FlattenPubKeySlice(o.Details().NotAllowedNodeIDs)
		tlvRecords = append(
			tlvRecords, tlv.MakePrimitiveRecord(
				notAllowedNodeIDsType, &bytes,
			),
		)
	}
